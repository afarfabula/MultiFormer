...
class ViT_Pose_Decoder(torch.nn.Module):
    def __init__(self, encoder : MAE_Encoder, keypoints=36, coor_num=1, token_num=285, dataset='mmfi-csi', num_person=1) -> None:
        super().__init__()
        self.keypoints = keypoints  # 改为36个点
        self.coor_num = coor_num    # 改为1维坐标
    ...
    def forward(self, img):
        ...
        # 修改输出格式为36×1
        pose = self.fc(x)
        pose = pose.view(batch_num, -1)  # 展平为36×1格式
...
class UnifiedPoseModel(torch.nn.Module):
    def __init__(self,
                 image_size=(114,10),
                 patch_size=(2,2),
                 emb_dim=256,
        ...
        self.pos_embedding = torch.nn.Parameter(torch.zeros((image_size[0]//patch_size[0])*(image_size[1]//patch_size[1]), 1, emb_dim))
        self.patchify = torch.nn.Conv2d(input_dim, emb_dim, (patch_size[0], patch_size[1]),(patch_size[0], patch_size[1]))
        self.transformer = torch.nn.Sequential(*[Block(emb_dim, encoder_head) for _ in range(encoder_layer)])
        self.layer_norm = torch.nn.LayerNorm(emb_dim)
        
        # Pose Decoder部分
        self.keypoints = keypoints
        self.coor_num = coor_num
        self.pose_prompt = nn.Parameter(torch.zeros(emb_dim, keypoints*num_person))
        
        # 输出层
        self.fc = nn.Sequential(
            nn.Linear(emb_dim, emb_dim//4),
            nn.ReLU(),
            nn.Linear(emb_dim//4, coor_num)
        )
    ...
    def forward(self, img):
        ...
        # 生成姿态点
        x = features.unsqueeze(2).expand(batch_num, -1, self.keypoints)
        x = x + self.pose_prompt.unsqueeze(0).expand(batch_num, -1, -1)
        pose = self.fc(x.permute(0,2,1))
        
        # 输出36×1格式
        return pose.view(batch_num, -1)
class UnifiedPoseModel2(torch.nn.Module):
    def __init__(self,
                 image_size=(114,10),
                 patch_size=(2,2),
                 emb_dim=256,
        ...
        # 初始化姿态解码器
        self.pose_decoder = ViT_Pose_Decoder(
...
if __name__ == '__main__':
    ...
    # 测试姿态解码器
    model = UnifiedPoseModel2(keypoints=36, coor_num=1)
    output = model(img)  # 输出形状为(batch_size, 36)
    print(output.shape)  # 打印前5个关键点的坐标
