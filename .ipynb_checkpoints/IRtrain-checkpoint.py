...
learning_rate = 0.0001*0.5*0.5

def getMinibatch(file_names):
    file_num = len(file_names)
    csi_data = torch.zeros(file_num, 3, 128, 128)
    ...
    nostage_GT_paf = torch.zeros(file_num, 38, 36, 36)
    ...
    nostage_GT_heatmap = torch.zeros(file_num, 19, 36, 36)

    for i in range(file_num):
        #print(file_names[i])
        #data = hdf5storage.loadmat(file_names[i], variable_names={'CSI', 'SkeletonPoints'})
        csi_data[i, :, :, :] = torch.from_numpy(TerminalCSIInput(file_names[i])).type(torch.FloatTensor)
        #print(csi_data[i, :, :, :])
        #csi_data[i, :, :, :] = torch.randn( 3, 128, 128)
        #csi_data_phase[i, :, :, :] = torch.from_numpy(PhaseExtract_dir(file_names[i])).type(torch.FloatTensor)
         # 假设 OppMatProcess 返回一个包含两个 NumPy 数组的元组 (PAF, heatmap)
        GT_paf_st1, pcm1 = OppMatProcess_ST1(file_names[i])
        GT_paf_st2, pcm2 = OppMatProcess_ST2(file_names[i])
        GT_paf_st3, pcm3 = OppMatProcess_ST3(file_names[i])
        nostage_GT_heatmap[i, :, :, :] = PcmReconstruct(pcm1,pcm2,pcm3)
        nostage_GT_paf[i, :, :, :] = torch.cat([GT_paf_st1, GT_paf_st2,GT_paf_st3], 1)
...
wisppn = ResNet_IRNK(ResidualBlock, [0, 0, 0 ,0])
...
for epoch_index in range(num_epochs):
 ...
 #eval_value = evalue_all_batch(test_mats, wisppn)
    ...
    for batch_index in range(batch_num):
        ...
        csi_data, nostage_GT_heatmap,nostage_GT_paf = getMinibatch(file_names)
        ...
        nostage_GT_heatmap = Variable(nostage_GT_heatmap.cuda())
        nostage_GT_paf = Variable(nostage_GT_paf.cuda())
        ...
        paf4,pcm4,paf3,pcm3,paf2,pcm2,paf1,pcm1= wisppn(csi_data, WGI,WGI)
        # 将列表中的张量合并成一个单一的张量
        #paf4,pcm4,paf3,pcm3,paf2,pcm2,paf1,pcm1= wisppn(csi_data,csi_data,csi_data )
        #print('pcm输出维度',pcm3.shape,pcm2.shape,pcm1.shape)
        #print('paf输出维度',paf3.shape,paf2.shape,paf1.shape)
        #loss = criterion_L2(paf4, nostage_GT_paf)+criterion_L2(paf3, nostage_GT_paf)+criterion_L2(paf2, nostage_GT_paf)+criterion_L2(paf1, nostage_GT_paf)+criterion_L2(pcm1, nostage_GT_heatmap)+criterion_L2(pcm2, nostage_GT_heatmap)+criterion_L2(pcm3, nostage_GT_heatmap)+criterion_L2(pcm4, nostage_GT_heatmap)
        loss1 = criterion_L2(paf1, nostage_GT_paf)+criterion_L2(pcm1, nostage_GT_heatmap)
        loss2 = criterion_L2(paf2, nostage_GT_paf)+criterion_L2(pcm2, nostage_GT_heatmap)
        loss3 = criterion_L2(paf3, nostage_GT_paf)+criterion_L2(pcm3, nostage_GT_heatmap)
        loss4 = criterion_L2(paf4, nostage_GT_paf)+criterion_L2(pcm4, nostage_GT_heatmap)
