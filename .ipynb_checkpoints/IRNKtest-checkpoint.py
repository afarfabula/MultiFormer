...
def evalue_all(mat_file,wisppn):
    ...
    csi_data = torch.zeros(1, 3, 128, 128)
    GT_heatmap_st1 = torch.zeros(1, 6, 36, 36)
    GT_heatmap_st2 = torch.zeros(1, 6, 36, 36)
    GT_heatmap_st3 = torch.zeros(1, 7, 36, 36)

    csi_data[0,:,:,:] = torch.from_numpy(TerminalCSIInput(mat_file)).type(torch.FloatTensor)
    ...
    Mconv7_stage6_L2 = pcm3.cpu().detach().numpy()
...
if __name__=="__main__":
    ...
    mats = []
    mats += glob.glob('/hy-tmp/group2_30_percent_bend/*.mat')
    #print(mats)
    mats += glob.glob('/hy-tmp/group2_30_percent_crouch/*.mat')
    mats += glob.glob('/hy-tmp/group2_30_percent_lean/*.mat')
    mats += glob.glob('/hy-tmp/group2_30_percent_push/*.mat')
    mats += glob.glob('/hy-tmp/group2_30_percent_sit/*.mat')
    mats += glob.glob('/hy-tmp/group2_30_percent_stand/*.mat')
    mats += glob.glob('/hy-tmp/group2_30_percent_walk/*.mat')
    mats += glob.glob('/hy-tmp/group2_30_percent_wave/*.mat')
    #print(mats)
    #mats = glob.glob('C:/Users\86183\Desktop/result2/result2/*.mat')
    import random
    #mats=mats[30:720:47]
    #mats = random.sample(mats, 100)
    #print(mats)
