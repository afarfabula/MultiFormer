...
class Channel_Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.flatten(2)
        #print('transpose前',x.shape)
        x = x.transpose(-1, -2)  # (B, n_patches, hidden)
        #print('ebedding前',x.shape)
...
class ChannelTransformer(nn.Module):
    def __init__(self, vis, img_size, channel_num, num_layers, num_heads):
        ...
        self.encoder2 = Encoder(vis, channel_num, num_layers, num_heads)
        self.encoder3 = Encoder(vis, channel_num, num_layers, num_heads)
        self.encoder4 = Encoder(vis, channel_num, num_layers, num_heads)
        self.encoder5 = Encoder(vis, channel_num, num_layers, num_heads)
        self.encoder6 = Encoder(vis, channel_num, num_layers, num_heads)
        self.encoder7 = Encoder(vis, channel_num, num_layers, num_heads)
        self.encoder8 = Encoder(vis, channel_num, num_layers, num_heads)
    ...
    def forward(self,en1):
        #en1 = self.reconstruct_1(en1)
        # 将 x 重塑为 [1, 128, 3, 12, 36]
        
       

        # 打印结果以验证形状
        #print('en1的维度',x.shape)
        #print('en1的维度',en1.shape)
        ...
        encoded1, attn_weights = self.encoder2(encoded1)  # (B, n_patch, hidden)
        encoded1, attn_weights = self.encoder3(encoded1)  # (B, n_patch, hidden)
        encoded1, attn_weights = self.encoder4(encoded1)  # (B, n_patch, hidden)
        
        encoded1, attn_weights = self.encoder5(encoded1)  # (B, n_patch, hidden)
        encoded1, attn_weights = self.encoder6(encoded1)  # (B, n_patch, hidden)
        encoded1, attn_weights = self.encoder7(encoded1)  # (B, n_patch, hidden)
        encoded1, attn_weights = self.encoder8(encoded1)  # (B, n_patch, hidden)
...
class ChannelTransformer_OPP(nn.Module):
    def __init__(self, vis, img_size, channel_num, num_layers, num_heads):
        super().__init__()
...
if __name__ == "__main__":
    x = torch.randn(2, 128, 36, 36)
    model = ChannelTransformer(vis=False, img_size=[36, 36], channel_num=128, num_layers=1, num_heads=3)
    y,_ = model(x)
    print(y.shape)
