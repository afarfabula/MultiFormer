...
def complex_upsample_axis(arr, up_length, axis=0):
    ...
    # 上采样倍数计算 (up/down = 128/10)
    upsampled = np.apply_along_axis(
        lambda x: resample_poly(x, up=up_length, down=original_length, window=('kaiser', 5.0)),
        axis=axis,
        arr=arr
    )
    return upsampled
...
def TerminalCSIInput(mat_file):
    ...
    upsampled_at1 = complex_upsample_axis(at1, up_length=128, axis=0)  # 结果 (128,30)
    upsampled_at2 = complex_upsample_axis(at2, up_length=128, axis=0)
    upsampled_at3 = complex_upsample_axis(at3, up_length=128, axis=0)

    upsampled_at1 = complex_upsample_axis(upsampled_at1, up_length=128, axis=1)  # 结果 (128,30)
    upsampled_at2 = complex_upsample_axis(upsampled_at1, up_length=128, axis=1)
    upsampled_at3 = complex_upsample_axis(upsampled_at1, up_length=128, axis=1)
...
if __name__=="__main__":
    ...
    mats = []
    mats += glob.glob('/hy-tmp/group1_70_percent_bend/*.mat')
    mats += glob.glob('/hy-tmp/group1_70_percent_crouch/*.mat')
    mats += glob.glob('/hy-tmp/group1_70_percent_lean/*.mat')
    mats += glob.glob('/hy-tmp/group1_70_percent_push/*.mat')
    mats += glob.glob('/hy-tmp/group1_70_percent_sit/*.mat')
    mats += glob.glob('/hy-tmp/group1_70_percent_stand/*.mat')
    mats += glob.glob('/hy-tmp/group1_70_percent_walk/*.mat')
    mats += glob.glob('/hy-tmp/group1_70_percent_wave/*.mat')
    #print(mats)
    #print(mats)
